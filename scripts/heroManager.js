...
{ 
    let touchedSpiders = inGameSpiders.filter (
        function (e)
        {
            ...
            if (bullet.orientation.horizontal == -1 && bullet.orientation.vertical == 1) 
            { 
                if(bulletCoords.left >= spiderCoords.left &&
                    bulletCoords.left + bullet.htmlElement.width() <= 
                    spiderCoords.left + (e.htmlElement.width() * e.PV) &&
                    bulletCoords.top + bullet.htmlElement.height() >= 
                    spiderCoords.top && bulletCoords.top <=
                    spiderCoords.top + (e.htmlElement.height() * e.PV)) 
                { 
                    return true; 
                } 
            } 
            if (bullet.orientation.horizontal == 1 && bullet.orientation.vertical == -1) 
            { 
                if (bulletCoords.left + bullet.htmlElement.width() >=  
                    spiderCoords.left && bulletCoords.left <= 
                    spiderCoords.left + (e.htmlElement.width() * e.PV) && 
                    bulletCoords.top <= spiderCoords.top + (e.htmlElement.height() * e.PV) && 
                    bulletCoords.top +      bullet.htmlElement.height() >= spiderCoords.top) 
                { 
    ...
    let touchedExtras = inGameExtras.filter(
        function (e)
        { 
            let bulletCoords = bullet.htmlElement.offset(); 
            ...
            if (bullet.orientation.horizontal == 1 && bullet.orientation.vertical == 1) 
            { 
                if (bulletCoords.left + bullet.htmlElement.width() >= extraCoords.left &&
                    bulletCoords.left <=extraCoords.left + (e.htmlElement.width()) && 
                    bulletCoords.top + bullet.htmlElement.height() >= extraCoords.top &&
                    bulletCoords.top <= extraCoords.top + (e.htmlElement.height())) 
                { 
                    return true; 
                } 
            } 
            if (bullet.orientation.horizontal == -1 && bullet.orientation.vertical == 1) 
            { 
                if (bulletCoords.left >= extraCoords.left &&
                    bulletCoords.left + bullet.htmlElement.width() <= extraCoords.left + (e.htmlElement.width()) &&
                    bulletCoords.top + bullet.htmlElement.height() >= extraCoords.top &&
                    bulletCoords.top <= extraCoords.top + (e.htmlElement.height())) 
                { 
                    return true; 
            ...
            if (bullet.orientation.horizontal == -1 && bullet.orientation.vertical == -1) 
            { 
                if (bulletCoords.left <= extraCoords.left + (e.htmlElement.width()) &&
                    bulletCoords.left + bullet.htmlElement.width() >= extraCoords.left &&
                    bulletCoords.top <= extraCoords.top + (e.htmlElement.height()) &&
                    bulletCoords.top + bullet.htmlElement.height() >= extraCoords.top) 
                { 
                    return true; 
